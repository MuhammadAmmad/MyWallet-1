...
public class ConsumptionMoney extends AppCompatActivity {
    ...
    private void setDefaultValue() {
        ...
        String query = "SELECT " + DatabaseHelper.CATEGORY_SPEND_COLUMN + " FROM "
                + DatabaseHelper.DATABASE_TABLE_WALLET +
                " WHERE " + DatabaseHelper.CATEGORY_SPEND_COLUMN + " IS NOT NULL ORDER BY " +
                DatabaseHelper.COLUMN_ID + " DESC LIMIT 1";

        Cursor cursor = db.rawQuery(query, null);
        if (cursor.moveToNext()) {
            TextView categoryDefault = (TextView) findViewById(R.id.txtCategory);
            categoryDefault.setText(cursor.getString(cursor.getColumnIndex(DatabaseHelper.CATEGORY_SPEND_COLUMN)));
        }
        cursor.close();
