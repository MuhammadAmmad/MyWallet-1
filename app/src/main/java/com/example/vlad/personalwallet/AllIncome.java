...
public class AllIncome extends AppCompatActivity {
    ...
    /**
     * Reads from database all categories of income and the amount.
     * Returns map where key is the name of category, and value is the total amount of income in this category.
     */
    private Map<String, Double> getAllCategoryAndSum() {
        ...
        String query = "SELECT " + DatabaseHelper.CASH_COLUMN + ", " + DatabaseHelper.CATEGORY_INCOME_COLUMN +
                " " + " FROM " + DatabaseHelper.DATABASE_TABLE_WALLET
                + " WHERE " + DatabaseHelper.CATEGORY_INCOME_COLUMN + " IS NOT NULL";

        Cursor cursor = mSqLiteDatabase.rawQuery(query, null);

        while(cursor.moveToNext()){
            String catName = cursor.getString(cursor.getColumnIndex(DatabaseHelper.CATEGORY_INCOME_COLUMN));
    ...
    private void createLayout(Map<String, Double> allCategoryAndSum) {
        TableLayout tableLayout = (TableLayout)findViewById(R.id.container_income);
        TextView allCashView = (TextView)findViewById(R.id.allCashIncome);
